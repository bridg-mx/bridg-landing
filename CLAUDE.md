...
- **Waitlist flow**: `WaitlistForm` (client component) → POST `/api/waitlist` (validation, honeypot field `website`, email dedup) → `src/lib/waitlist-store.ts` → Prisma (`src/lib/prisma.ts`) → Supabase Postgres. Duplicate emails map from Prisma error `P2002` to `{ error: "duplicate" }` (HTTP 409). Signups are viewed in the Supabase Table Editor (no admin UI).
- **Database / Prisma**: schema in `prisma/schema.prisma` (model `WaitlistSignup` → table `waitlist_signups`); migrations in `prisma/migrations/` are the source of truth (replaces the old `supabase/schema.sql`). Prisma 7: the generated client lives in `src/generated/prisma/` (gitignored, regenerated via the `postinstall` script) and **requires a driver adapter** — runtime uses `@prisma/adapter-pg` with `DATABASE_URL`. Datasource for the CLI is set in `prisma.config.ts` (not in the schema), pointing at `DIRECT_URL`.
  - **Two connection strings** (Supabase pooler is IPv4; the direct `db.*.supabase.co` host is IPv6-only and unreachable on IPv4 networks): `DATABASE_URL` = transaction pooler (`...pooler.supabase.com:6543`, `?pgbouncer=true`) for runtime; `DIRECT_URL` = session pooler (`...:5432`) for migrations (the transaction pooler can't run migrations).
  - Scripts: `npm run db:migrate` (dev), `npm run db:deploy` (apply committed migrations), `npm run db:generate`, `npm run db:studio`.
